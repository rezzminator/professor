...
Trigger chain: `.claude/settings.json` Stop hook → `guard-stamp.sh stop` + `codex-sync.sh sync` (timeout 60). `codex-sync.sh`: `mark` (PostToolUse Edit|Write sets `tmp/professor_codex_dirty`), `sync` (Stop: both `pfm codex build|check` and `pfm opencode build|check`; failure blocks turn end, exit 2, and names the failed stage). `.claude/scripts/dev.sh` runs `pfm opencode build|check|doctor` and names the native repair command on failure.
...
- Both mirrors → `pfm codex check` / `pfm opencode check|doctor` → terminal; on failure `codex-sync.sh sync` blocks Stop (exit 2).
