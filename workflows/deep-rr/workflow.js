...
const WEB_ONLY = `
Use the web only (WebSearch / mcp__harvester__readPage / mcp__harvester__readWork) to check sources — never read local files or this repo's own code; they are not evidence.`;
...
TOP GOAL: "{{query}}".
Tools (load any missing via ToolSearch): WebSearch; mcp__harvester__webSearch; mcp__harvester__findWorks — finds a work/DOI's open-access candidates, each read via readWork(handle); mcp__harvester__readPage — fetches + caches a web url; mcp__harvester__readWork — fetches + caches a work (DOI, arXiv id, PMID, PMCID, ISBN, findWorks handle). Built-in WebFetch is denied; fetch only through Harvester.
...
1. DISCOVER — run ALL lanes' searches in ONE parallel batch (WebSearch / mcp__harvester__webSearch / findWorks). Prefer each lane's assigned venues; let its \`note\` decide which results serve it. A lane carrying a concrete ref takes that ref as a source directly — no search needed for it.
2. SIZE — call mcp__harvester__readPage (web urls) / mcp__harvester__readWork (works: DOI, id, findWorks handle) with size_only:true on EVERY candidate across all lanes in ONE parallel batch. With size_only it fetches + caches the full text and returns {tokens (token_count is the same count), path to the cache file, chars, cache_status, and partial when the text is incomplete} and NO body. Drop any candidate that failed or came back walled/thin and pick another from the same lane.
...
For each lane, return its chosen sources as {source (the exact url or DOI), path (the cache path from size_only), size (the reply's tokens), chars}. Group them under the lane's id. A lane may return several sources; return an empty list for a lane only when every candidate failed.{{translateClause}}{{researcherClause}}{{vocabClause}}{{corruptClause}}
