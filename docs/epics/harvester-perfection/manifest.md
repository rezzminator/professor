...
7. The surface redesign, built from `surface-spec.md` in five tasks (R1 central retrieval, R2 tools, R3 docs lanes CLI, R4 deep-rr, R5 templates). Moved ahead of the failure messages: R2 rewrites every error string that names an old tool, so F7 works on the new names once. R1 landed without the file policy's browser-download rung: the browser adapter returns rendered HTML only and cannot hand back a download's bytes, so that rung is R1b (a download-capture method on the adapter and its Python side), after R2; provider artifacts still come back in memory rather than streamed.
