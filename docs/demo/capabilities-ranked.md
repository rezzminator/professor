...
Built from five tracer inventories (read from disk, 5 tracers dispatched → 5 received; four remain beside this file — the discipline-layer inventory was removed and its rows cite the source files directly) plus `research-blocked-web-percent.md`. Rank = (nobody else ships it) × (visible on stage in 60 s) × (the AI Builders audience's own pain). Evidence pointers name the inventory file and the code anchor.
