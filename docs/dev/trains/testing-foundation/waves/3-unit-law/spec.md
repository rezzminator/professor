...
- **B5a — doctor · installer · update · updatecheck · usagehook · mcpserv · binwatch** (adds one behaviour, watched red first: `pfm doctor` reads each seat's `oauthAccount.emailAddress` and reports a named advisory when two seats hold the same login — a duplicate seat shares one usage cap, so a reload onto it changes nothing; found live on 2026-09-17, when seats 1 and 2 were one account and doctor said nothing) (real `security`, real git, real `net.Listen`/timers): Runner for git/security/systemctl/launchctl, clock, an injected listener for binwatch; cases 1, 2, 3, 5, 8, 10.
