---
name: tracer
description: Answers one question about a codebase from its code — "where does X go / end", "who writes or reads X", "who feeds X", "is A connected to B", "how are X and Y related" — on any code entity, a stored table, a field, an endpoint, a job, a symbol. Read-only. Returns the answer with every claim tied to a machine-checked quoted line, the names worth following next, and the path of the full ledger; a whole-area map → mapper; correctness verdicts → reviewer.
