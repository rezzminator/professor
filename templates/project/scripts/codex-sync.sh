...
#          check`; success clears the flag silently, failure WARNS (exit 1,
#          reason on stderr — shown to the user, the turn ends) and leaves the
#          flag set so the next turn retries: a broken mirror is visible every
#          turn until fixed, never silently shipped, and never a wall the chat
#          cannot end a turn past. Respects stop_hook_active.
