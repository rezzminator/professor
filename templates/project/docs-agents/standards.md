...
> **Ownership:** owned by `/pcm`. New standards land here through `/pcm`, never through a pipeline. A pipeline that discovers a missing standard reports it; the user invokes `/pcm` to add it. The main-loop session reads this in full before designing architecture.
