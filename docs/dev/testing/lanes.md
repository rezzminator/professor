...
- `blocked` a declared precondition beat failed (`blocked-by <beat>`) — never `✗` for someone else's failure, and never silence.
...
4. Write the beat in `infra/fence/lanes/<lane>.sh` using only `lib.sh`: `beat <id> <landscape-ids…>`, `spends <seat>`, `target <chat>`, then exactly one of `pass` / `fail` / `known` / `blocked`. Assert from pfm's own report or the pane — never from what a model said.
