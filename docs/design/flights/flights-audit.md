...
| `run.md` | the flight directory | The header's baseline sha and date; one line per event with its token; the resume point |
...
| Executor transcripts, nested and live | `$CLAUDE_CONFIG_DIR/projects/{cwd slug}/{session id}/subagents/agent-*.jsonl` (default `~/.claude`), one flat directory whatever the depth; the `.meta.json` beside each names its `agentType` and `spawnDepth` | Calls per executor, peak context, files read outside the spec, reporting cadence, poll chains; the `/code-review low` run and its findings |
...
2. Ledger truth: every `DONE` line's task shows changed files inside its index `files` in `git diff {baseline}`, and nothing outside; its transcript shows the `/code-review low` run, and every finding inside its files is fixed in the diff; the covering test per `Done when` row exists, was read, and its run is in a log or a transcript, not in a claim; a `CLAIMED` line with no verdict is in flight (a live executor) or lost (named); a `FAILED` or `SPEC-DRIFT` line has its revising round (new or changed task files, a rebuilt index); a `STALE` line has a seat status behind it.
3. Faults handled the only legal way: a task file changed after the header date only by a revising `flights-speccer` call (the orchestrator's transcript shows the call); no rulings, notes or patches written beside the directory; no task file re-dispatched unchanged after `FAILED`.
...
7. Landing: the checks were watched (the transcript shows the command and its output, not a summary); every review finding is terminal (fixed, or named in the return and carried to `NOTES`); the commit sha exists and its diff matches the flight's diff; nothing outside the flight's files changed on the branch.
...
| The orchestrator | [`flights-orchestrator`](flights-orchestrator.md) | The `run.md` shape and tokens the audit reads; the return the audit checks; the review order in the brief |
