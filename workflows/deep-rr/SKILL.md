...
**Preflight — the fetch MCP (Harvester) must be live.** RR fetches only through `mcp__harvester__read` (web pages in `urls`, papers and books in `publications`) (built-in WebFetch is hook-denied), so a missing or dead server makes every fetch error and yields a snippet-only run. Before launching, check for the `mcp__harvester__*` tools (via ToolSearch):
...
- **Present** — smoke-test with one `mcp__harvester__read` with a stable URL in `urls` (`https://example.com`); a clean read means go, an error means have the user reconnect (`/mcp`) or restart the server — hold until it passes.
