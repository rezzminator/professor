...
func TestReloadCommandLinksIntoEverySeat(t *testing.T) {
	...
	installer := &engine{
		options: Options{Mode: ModeApply, Home: home, ConfigDir: primary, ConfigDirs: []string{primary, second}, Stdout: io.Discard},
		apply:   true, managedRoot: filepath.Join(home, "managed"),
	}
	if err := os.MkdirAll(installer.managedRoot, 0o700); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(installer.managedRoot, "reload.command.md"), []byte("# reload\n"), 0o600); err != nil {
		t.Fatal(err)
	}
