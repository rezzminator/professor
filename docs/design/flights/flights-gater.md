...
3. `/code-review {effort}` over the flight's diff, at a level the gater sizes itself from `git diff {baseline} --stat` over the flight's files: up to 5 files and 200 changed lines `low`, up to 15 files and 800 lines `medium`, beyond either `high`; a `hard` task in the index raises it one step. The brief names no effort. Every finding inside the flight's files is fixed; one outside them is recorded untouched.
