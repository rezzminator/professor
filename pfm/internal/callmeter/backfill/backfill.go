...
func (s Summary) String() string {
	...
	fmt.Fprintf(&out, "callmeter backfill: %d transcripts read in %d config dirs, %d unreadable, %d malformed lines\n",
		s.TranscriptsRead, len(s.ConfigDirs), len(s.Unreadable), len(s.Malformed))
	fmt.Fprintf(&out, "calls: %d inserted, %d filled; requests: %d; agents: %d; entries older than the window: %d; retired %d\n",
		s.CallsInserted, s.CallsFilled, s.Requests, s.Agents, s.EntriesSkipped, s.Retired)
