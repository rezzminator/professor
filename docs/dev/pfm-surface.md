...
| ✚ `callmeter` | `pfm callmeter report {files\|writes\|commands\|context\|sequences\|faults} [--since D] [--project P] [--agent-type T] [--session S] [--config-dir DIR] [--limit N]` · `pfm callmeter backfill [--since D] [--config-dir DIR]` | Reads and fills the call store `$HOME/.local/state/pfm/callmeter.db` that `pfm internal callmeter` writes. `report` prunes rows older than the 30-day window, parses pending Bash commands, then prints one fixed-width table with a note line per named gap; chat names come from pfm's transcript index (`?` and one note when it cannot be read). An absent store prints `callmeter: no store at {path}: nothing recorded yet` and exits 0 without creating it; a store that cannot be opened prints `callmeter: cannot open store {path}: {error}` and exits 1. `backfill` prunes, then rebuilds rows from `projects/**/*.jsonl` (subagents included) at or after the later of `--since` and the window start, prints transcripts read/unreadable and calls inserted/filled, and exits 1 only when the store fails — an unreadable transcript is a summary line and a `backfill` fault. Both cover every Claude config dir the machine config names; `--config-dir` narrows to one and an unconfigured dir is a usage error naming them. A `--since` older than the window is clamped with one stderr note. |
