...
func TestProjectNameFilesAChatUnderItsRepository(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{"repo root", fixture.Repo, "intuita"},
		{"repo subdirectory", fixture.Subdir, "intuita"},
		{"worktree under .worktrees", fixture.Inner, "intuita"},
		{"worktree outside the repo", fixture.Outer, "intuita"},
		{"deleted worktree", filepath.Join(fixture.Repo, ".worktrees", "gone", "pfm"), "intuita"},
...
func TestLiveWorktreeChatLeadsUnderTheRepoProject(t *testing.T) {
	...
	if !found || row.Project != "intuita" || row.CWD != fixture.Inner {
		t.Fatalf("worktree seat row = %#v, want Project intuita with CWD %q", row, fixture.Inner)
	}
	if len(output.ProjectOrder) == 0 || output.ProjectOrder[0] != "intuita" {
		t.Fatalf("ProjectOrder = %q, want intuita first", output.ProjectOrder)
	}
