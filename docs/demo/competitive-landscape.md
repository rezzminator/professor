...
3. **The scope is very wide.** Fleet TUI, discipline templates, a document harvester with shadow-library providers, two research engines, a verification engine, a legal playbook shelf, memory "dream" organs, a VS Code extension and themes. Each rival does one of these and explains it in a paragraph. The pitch doc itself has to rank 15 capabilities.
...
6. **Legal and ToS exposure.** Harvester fetches with a "Chrome-fingerprint TLS" rung and has opt-in LibGen and Sci-Hub providers (`pfm/internal/harvest/find_works.go:52`, `known_id.go:39`). Professor also drives multiple subscription accounts, and agentty's README flags that as an open ToS question. Professor's README addresses neither issue, and a company adopter will ask.
