...
- **Verification depth.** The flights pipeline has named failure states (a missing executor return is named, never silent; `flights-lander` counts no suite it did not watch run as a pass). The nearest peers are A Team's pipeline auditor and agentty's controller-verified research. Both are real but narrower, going by their READMEs.
