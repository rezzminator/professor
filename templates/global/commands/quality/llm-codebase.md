...
The design document is reviewed before anything is built: placement law, literal lists shadowing derived constants, coupled edits, checks whose broken state would read as PASS. In a project installed from this blueprint, `flights-speccer`'s reconcile phase performs that review, a design that changes what an LLM receives or returns goes through the project's RND gate, and code moves through `/flights:spec` and the flight executors, which apply this command's write-time laws (search before creating, one term per concept, the façade, no negation directory, the size ceiling, versioned scripts, one test home) on every file they write. Elsewhere, the owner assigns the review and the build; this command writes the design only.
