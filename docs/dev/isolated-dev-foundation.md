...
- An Xvfb display on `:99`, real fonts, and 2 GB of `/dev/shm`. The harvester's browser renders headless only, so it never opens a window on that display. Docker's 64 MB default crashes renderers on heavy pages.
