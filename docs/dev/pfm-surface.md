...
| ◆ `harvest` | `pfm harvest [--refresh] [--include-content=false] [--ocr-language latin\|zh\|ja\|ar\|ru\|he] [--json] [--header 'Name: value']... <url\|path\|identifier>...` | Multi-source read (each argument routed into `read`'s `urls`, `files` or `publications`) → markdown, cache + 24h TTL (`cache.ttlSeconds` in `harvester.config.json`). Stdout clips large sources; full docs live at the printed `path:`. |
...
| ◆ `harvest search` | `pfm harvest search [--type any\|paper\|book] [--limit N] [--json] <query>...` | Runs `search_literature`: ranked scholarly candidates, each with a `handle` to pass to `pfm harvest` or `read`'s `publications`, and each discovery source's status. |
...
| ✚ `callmeter` | `pfm callmeter report {files\|writes\|commands\|context\|sequences\|faults} [--since D] [--project P] [--agent-type T] [--session S] [--config-dir DIR] [--account N] [--limit N]` | Reads the call store `$HOME/.local/state/pfm/callmeter.db` that `pfm internal callmeter` writes. `report` prunes rows older than the 30-day window, parses pending Bash commands, then prints one fixed-width table with a note line per named gap; chat names come from pfm's transcript index (`?` and one note when it cannot be read). An absent store prints `callmeter: no store at {path}: nothing recorded yet` and exits 0 without creating it; a store that cannot be opened prints `callmeter: cannot open store {path}: {error}` and exits 1. It covers every Claude config dir the machine config names; `--config-dir` narrows to one and an unconfigured dir is a usage error naming them. `--account N` keeps the calls and requests that configured account ran, leaving out rows with no account, and an unconfigured id is a usage error naming the configured ids. A `--since` older than the window is clamped with one stderr note. |
| ● `run` | `pfm run …` | Chat-spawn plumbing (backs `chat new`). Unchanged. |
| ● `agent` | `pfm agent open …` | Headless claude open path. Unchanged. |
| ◆ `internal` | `pfm internal callmeter\|clear-kill\|explore-deny\|git-guard\|rr-dir\|epic-inject\|reload-intercept\|compact-nudge\|compact-gate\|exit-intercept\|exit-close` | Hook backends: ✚ `callmeter` (async on six events, records every Claude tool call into the call store `pfm callmeter` reads; section below), ✚ `explore-deny` (ports the shell script; stdin hook JSON → allow/deny), ✚ `git-guard` (PreToolUse, matcher `Bash`, every account; stdin hook JSON → deny a shared git write to every agent but `gitter`, per `docs/design/hooks/hooks.md` § git-guard), ✚ `rr-dir` (SubagentStart, matcher `rr\|super-rr\|heavy-rr`; stdin hook JSON `cwd` → one `additionalContext` line: `RR-DIR: {nearest ancestor}/.professor/RR`, the same with `(fallback: …)` for the clone's own ledger, or `RR-DIR-ERROR: {reason}`; exit 0 on every path), ✚ `epic-inject` (see below), ✚ `reload-intercept` (see below), ✚ `compact-nudge` (see below), ✚ `exit-intercept` (see below), ✚ `exit-close` (see below). |
