...
- Local (2026-09-18): the repo-local OpenCode compiler `.claude/scripts/build-opencode.mjs` is retired — `pfm opencode build|check|doctor` is the single writer of `.opencode/**` (testing-foundation Wave 8 item 2). `codex-sync.sh` compiles both mirrors through pfm and, while `develop` stays uninstalled, prefers the fence-built `tmp/timing/pfm-dev-bin`; `dev.sh verify templates` builds that binary inside the fence and never installs. The earlier KEEP-LOCAL notes on `build-opencode.mjs` above are history.
- Local (2026-09-17): the fence moved to `infra/fence/` (`docker-compose.yml`, `pfm-dev.Dockerfile`, `fence-env.sh`, `tools.env`, `tools.sh`, `release-rehearsal.sh`); `infra/demo/` keeps only the demo, `infra/check-self-hosted-manifest.sh` stays at the top level (repo gate, not fence-only), and `infra/readme-gif/` is retired whole — the mocked recorder and every pointer to it, `.claude/commands/readme-gif.md` included. `docs/img/pfm-fleet.gif` and its `README.md` embed are removed with the recorder (GOD's call on the user's "no more faking" — a dead artifact nothing can regenerate); `infra/readme-cards/` stays, its fate an open question for the user.
- Local (2026-09-18): `dev.sh verify templates` generates both engine mirrors from the Claude sources (the tree's own `go run ./cmd/pfm codex build`, then `build-opencode.mjs generate`) before the marker, roster, opencode and manifest gates read them — the mirrors (`AGENTS.md`, `pfm/AGENTS.md`, `.codex/{agents,skills}`, `.opencode/**` outputs) and the global `.toml` twins left git, so a fresh clone holds none until it generates. The adopter twin `templates/project/scripts/dev.sh` has no `templates` project and is unchanged.
- Local (2026-09-18): `.claude/scripts/codex-sync.sh sync` adds `~/.local/share/mise/shims` and `~/.local/bin` to PATH when `node` is absent — the Stop hook runs with the harness's PATH, so a mise-installed node reported "node not found" and the OpenCode half of every post-edit sync failed. The adopter twin `templates/project/scripts/codex-sync.sh` compiles the Codex mirror only and is unchanged.
- CORRECTION (2026-09-18, fix wave L3-F16): the two entries directly above no longer match the scripts on disk. `.claude/scripts/codex-sync.sh` today has no `mise`, `PATH=`, or `node` reference at all (grep confirms zero hits) — the mise-shim PATH fix either never landed or was since removed; whichever it was, it is not there now. `dev.sh verify templates`'s "templates — generate the engine mirrors" step runs `go run ./cmd/pfm codex build` then `go run ./cmd/pfm opencode build` — never `build-opencode.mjs generate`, which the 2026-09-18 entry two above this one already retired the same day. Appended rather than rewritten per this ledger's own append-only header.
- Local (2026-09-19): `dev.sh test pfm` no longer passes on tests it never ran — `skip_gate` runs `pfm/scripts/skip-check.sh` over each `go test -json` artifact, and a skipped test that `pfm/scripts/known-skips.tsv` does not name is a `GAP` and a non-zero exit (the previously caller-less `gap_step` is its verdict; the unread `SWEEP_ALL` is removed). `verify pfm` runs the gate scripts' own `pfm/scripts/*_test.sh`; `verify templates` runs `infra/fence/lanes/check-map.sh --no-derive` and the lane library's `tests/*_test.sh`. `repo_git` has one definition, `pfm/scripts/repo-git.sh`, sourced by `dev.sh` and both arch ratchets; the shell tests share one harness, `scripts/shtest.sh`. Repo-local gates over `pfm/` and the lanes — no adopter twin.
- Local (2026-09-21): root `CLAUDE.md` § Repo structure names the rr design cluster beside the flights one — `design/` "the flights family under `design/flights/`, the rr family under `design/RR/`". A roster fact of this repo's own docs tree; the adopter `templates/project/CLAUDE.md` carries no `docs/design/` and is unchanged. The rr-family rework it points at (`templates/global/agents/rr.md`, `sub-rr.md`) is machine-global and release-bound, not drift.
- Local (2026-09-21): `.claude/agents/tracer.md` carries the machine-global tracer body verbatim (the ledger design: `ledger.py` + `scribe` readers) — the local delta is the `description:` alone, whose trigger list names this repo's own entities (placeholder tokens, templates, hooks, Go/TS symbols); the walker agents it used to spawn (`sub-tracer`) are deleted upstream.
