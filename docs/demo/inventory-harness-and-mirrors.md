...
Trigger chain: `.claude/settings.json:57-73` Stop hook → `guard-stamp.sh stop` + `codex-sync.sh sync` (timeout 60). `codex-sync.sh`: `mark` (PostToolUse Edit|Write sets `tmp/professor_codex_dirty`), `sync` (Stop: `pfm codex build` + `check` AND `build-opencode.mjs generate` + `check`; failure blocks turn end, exit 2, names the failed stage). `.claude/scripts/dev.sh:293-297` runs `build-opencode.mjs check && doctor`, failing with "opencode mirror FAILED — run: node .claude/scripts/build-opencode.mjs generate".
...
- Both mirrors → `pfm codex check` / `build-opencode.mjs check|doctor` → terminal; on failure `codex-sync.sh sync` blocks Stop (exit 2).
