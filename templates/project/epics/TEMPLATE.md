...
2. **Consolidate** — for a session save, walk the ENTIRE conversation, not just recent turns; for a flight, the merged diff + report:
   ...
   - One `## Progress Log` milestone line; new epic files registered in `## Files`; add to `pipelines:`/`flights:` as applicable; bump `updated:`.
...
- {YYYY-MM-DD} — {pipeline|flight|session}: {one sentence} ({SHA})
