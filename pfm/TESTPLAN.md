...
| picker/name-sync live scans seed empty Codex pane bindings but never overwrite a hook-supplied post-clear id | JAIL | `cmd/pfm/internal_clear_kill_jail_test.go`, `cmd/pfm/ls_pipeline.go` | |
...
| an UNREADABLE kill table never drops a binding, and an explicit `pfm chat kill` (no prompt baseline) never does either | JAIL | `cmd/pfm/codexpanes_test.go`, `cmd/pfm/ls_pipeline.go` | |
...
| Hiding a LIVE chat (resolved socket+pane) also runs the `--exit` finisher — CLI `chat kill <id\|self>`, picker ⌃X, and MCP `chat_kill` all resolve the address and hit the same `kill.Manager.Kill` branch; a resumable-only target stays a store write | JAIL+tmux | `kill/manager.go:86-145`, `cmd/pfm/ls_pipeline.go` (`resolveRowTarget`), `ui/model.go` (`toggleKilled`), `mcpserv/actions.go` (`chatKill`) | |
...
| 9 | **Primary-account round trip** (`ls_pipeline.go:553-631`, `fleetdb/fleetdb.go:441-482`, `internal/installer/assets/shim/pfm.zsh`) | The store and launcher must enforce the same account roster. |
