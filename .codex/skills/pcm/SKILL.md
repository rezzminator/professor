---
# Generated by pfm codex build from .claude/commands/pcm.md; do not edit — edit the source, then re-run: pfm codex build
name: pcm
...
- **Sub-agents** (`.claude/agents/*.md`): frontmatter `name` (kebab-case), `description` (§ Descriptions — it carries the auto-delegation routing weight), `tools` (minimal allowlist), `model: inherit|gpt-5.6-sol|gpt-5.6-luna|gpt-5.6-luna`. Body IS the system prompt — role sentence, numbered procedure, short checklist, output format; subagents see only their own prompt + env.
