...
2. `readWork` for a paper, book chapter, PDF or document identifier, `findWorks` first when only a title is known. WebFetch is the default for a page.
3. `readPage` when a WebFetch fails or returns an encoded or empty body.
