...
| ◆ `internal` | `pfm internal clear-kill\|explore-deny\|epic-inject\|reload-intercept\|compact-nudge\|exit-intercept\|exit-close` | Hook backends: ✚ `explore-deny` (ports the shell script; stdin hook JSON → allow/deny), ✚ `epic-inject` (see below), ✚ `reload-intercept` (see below), ✚ `compact-nudge` (see below), ✚ `exit-intercept` (see below), ✚ `exit-close` (see below). |
