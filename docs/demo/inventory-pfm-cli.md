...
| `pfm statusline [--refresh-gpt \| --subagents]` | Renders the native Claude Code status line from stdin JSON; `--subagents` renders the agent panel's per-row bodies from `subagentStatusLine` JSON | `pfm/cmd/pfm/main.go:82-83`, `pfm/cmd/pfm/statusline_command.go:24` | No — hook-only surface, standard status-line contract |
...
| `pfm internal compact-gate` | PreCompact hook: blocks an automatic compaction (exit 2) while the compacting party — the main chat or the sub-agent inferred from transcript mtimes — is below its own `claude.autoCompactMain` / `claude.autoCompactSubagent` threshold; fail-open (stdin-driven) | `pfm/cmd/pfm/main.go`, `pfm/internal/compactgate/compactgate.go` | Yes — per-party compaction thresholds Claude Code itself does not offer (a sub-agent inherits its parent's window) |
