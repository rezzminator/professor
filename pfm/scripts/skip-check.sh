...
# Prints one `GAP skipped test: <pkg> <Test> [<class>]` line per listed skip,
# one `SKIP-UNLISTED <pkg> <Test>` line per unlisted one, then the verdict:
...
while IFS=$'\t' read -r pkg test; do
  ...
  if [ -n "$class" ]; then
    printf 'GAP skipped test: %s %s [%s]\n' "$pkg" "$test" "$class"; listed=$((listed + 1))
  else
