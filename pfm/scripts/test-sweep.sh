...
if [ "$MODE" = report ]; then
  ...
  DOC_TMP="$(mktemp "${DOC}.XXXXXX")" || { echo "SWEEP-ERROR: could not create temporary report beside $DOC" >&2; rm -rf "$RT"; rm -f "$BODY"; exit 2; }
  {
    ...
    echo "behind the two 1-D sweeps plus shuffle validation: this script's own header, and"
    echo "docs/dev/trains/testing-foundation/waves/2-timing-ratchet/spec.md item 3."
    echo
    cat "$BODY"
  } > "$DOC_TMP"
