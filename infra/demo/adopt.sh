...
*)
  ...
  pfm chat new --name "$CHAT" --engine cc --account "$SEAT" --cwd "$DIR" --await --settle 60 --timeout 1500 \
    "pfm init has scaffolded Professor into this repository ($NAME). Follow /worktree/docs/SETUP.md § Install interview end to end, Phase 1 through Phase 3. Use these answers and do not ask them again: project identity: '$PITCH'; character: keep Professor; roster: single project, this repo, language from its package manifest; tech stack: as the repo shows; test command: '$TEST_CMD'; Tier B opt-ins: none; Codex dual-runtime: yes (the deck shows the compile hooks: .codex mirror, codex-sync.sh on PostToolUse and Stop, every {CODEX_*} token filled); sacred ground: none beyond the defaults; ports: none. Treat 'go' as already typed. Fill every registered token from /worktree/docs/PLACEHOLDERS.md in the scaffolded files, write .professor/manifest.json, run the smoke test, and finish with one line: INSTALLED plus the count of files you changed." >/dev/null || rc=$?
