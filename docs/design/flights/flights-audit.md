...
| Executor transcripts, nested and live | `$CLAUDE_CONFIG_DIR/projects/{cwd slug}/{session id}/subagents/agent-*.jsonl` (default `~/.claude`), one flat directory whatever the depth; the `.meta.json` beside each names its `agentType` and `spawnDepth` | Calls per executor against its 80-call cap, peak context, files read outside the spec, reporting cadence, poll chains |
