...
| Token | Meaning |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `JAIL` | Fully exercisable in a path jail: `internal/paths/paths.go` `Env*` overrides (`PFM_HOME`, `PFM_DB`, `PFM_FLEET_DB`, `PFM_SID_DIR`, `PFM_CLAUDE_ROOTS`, `PFM_CODEX_ROOT`, `PFM_TMUX_DIR`, `PFM_PROC_ROOT`) plus the test knobs `PFM_TEST_NOW_NS` (`internal/fleet/scan.go`) and `PFM_TEST_FRESH_SOCKET` (`internal/spawn/socket.go`). Reference harness: `pfm/testdata/e2e.sh`. |
...
| `JAIL+sh` | zsh/bash fixture with a fake `$HOME`, scratch db and an EMPTY socket dir. References: `internal/installer/shim/shim_test.go` and `internal/installer/installer_test.go`. Window-name convergence is covered by the Go `probe-*` tmux jails. |
...
| `pfm codex agents [--home PATH]` — no positional | JAIL | compiles `{home}/.professor/templates/global/agents/*.md` into the pfm-owned generated directory (`paths.GeneratedCodexAgentsDir`, never beside the source), installs sources into `{home}/.claude/agents` and the generated TOMLs into `{home}/.codex/agents`; a variant declared in `templates/global/agents/variants.json` (`{name: {from, <frontmatter key>: value}}`) is the source re-rendered with `name:` and the declared keys overridden, body byte-identical, into `paths.GeneratedClaudeAgentsDir`, then linked into every account and compiled to its own TOML like an original — a malformed declaration (unknown `from`, a key the source lacks, a name colliding with an original) aborts before any write; doctor counts a declared variant as owed, uninstall retires its links and that directory; a link still pointing at the retired in-clone twin re-points on the next run; escaping mirrors build-codex.mjs:151-153 byte-for-byte (verified against the retired host `build-global-agents.py`, since deleted) | `internal/codexgen/globalagents_test.go`, `internal/codexgen/globalvariants_test.go`, `cmd/pfm/codex_agents_command_test.go` |
...
| `ls --tsv` → TSVPicker, stable rows | JAIL | `internal/picker/picker.go`; golden `testdata/golden/ui.tsv` | |
| `ls -a/--all` → AllView | JAIL | `internal/picker/picker.go` | B2 |
| `ls -K/--killed` → the kill ledger (`id`, engine, timestamp), noninteractive | JAIL | `internal/picker/picker.go`, `main_test.go` | B2, B3 |
| `ls --all --killed` → rc 2 (mutually exclusive) | JAIL | `internal/picker/picker.go` | |
| `ls --plain --tsv` (or any two renderers) → rc 2 | JAIL | `internal/picker/picker.go` (`boolCount`) | |
| `ls <id>` → same as `chat open <id>` | JAIL | `internal/picker/picker.go` | |
| `ls <id>` combined with any flag → rc 2 | JAIL | `internal/picker/picker.go` | |
| `chat open <target>` on an unindexed id → `is not indexed`, rc 1 | JAIL | `chat_command.go`, `internal/picker/picker.go` | B3 |
| `chat open <target>` whose CWD vanished → falls back to `$PWD` for non-live kinds | JAIL | `internal/picker/picker.go` | |
...
| Codex 0.154 renames silently → the rename is PROVEN from `session_index.jsonl` (the name, stamped at or after the rename began), a proven rename is never retried, the pre-filled modal is found by its `Name thread`/`Rename thread` title, and an unreadable ledger reports "could not verify", never "unnamed"; ONE proof inside `RenameCodex`, fed once by `main` (`spawn.UseCodexHomes`), so every chat door (new, branch, post-`/clear` re-apply) is covered without carrying anything; the dialog is recognized only as its own gutter line with no composer on screen | JAIL | `TestCodexSilentRenameIsProvenFromTheIndex`, `TestCodexRenameOfANamedThreadFindsItsRetitledModal`, `TestCodexRenameThatCannotBeVerifiedSaysSo`, `TestRenameModalOpenReadsOnlyTheDialog`, `internal/spawn/rename_proof_test.go`, `internal/codexmeta/session_index_test.go` | live PING_PROBE launch reported unnamed |
...
| hidden `headless` compatibility alias exposes the public subcommand help without a stale deprecation warning | JAIL | `TestHeadlessCompatibilityAliasExposesPublicHelp` | |
| Name resolution: name, id-prefix, socket; live row beats its resume twin; ambiguity refused | JAIL | `TestMatchPrefersTheLiveSeat` | |
| `index` → counters line on stdout | JAIL | `cmd/pfm/index_command.go` (`formatCounters`) | B4 |
| `index --full` → reparse all + `last_full_index_at` meta | JAIL | `cmd/pfm/index_command.go` | B4 |
| `index --progress` → start/elapsed on stderr | JAIL | `cmd/pfm/index_command.go` | |
...
| canonical `pfm` executable read | JAIL | readable target-HOME binary is clean; unreadable or absent canonical binary is a warning and rc 1 | `internal/doctor/path_test.go` |
| PATH candidate resolution | JAIL | target-HOME canonical candidate first is clean; no target-HOME candidate or an in-home shadow is a warning and rc 1; host candidates are ignored | `internal/doctor/jail_test.go`, `internal/doctor/path_test.go` |
| PATH candidate hash | JAIL | matching target-HOME candidates are clean; target-HOME read failure or hash mismatch is a warning and rc 1 | `internal/doctor/jail_test.go`, `internal/doctor/path_test.go` |
...
| configured Claude seats sharing `oauthAccount.emailAddress` | JAIL | `pfm doctor` reports one deterministic advisory per duplicated login, names the seats and shared usage-cap consequence; a missing registry is absence, while malformed or unreadable registry content is a warning | `internal/doctor/config_checks_test.go` (`TestDoctorAdvisesWhenConfiguredSeatsShareOAuthLogin`, `TestDoctorCountsUnreadableSeatRegistryAsWarning`, `TestDoctorReportsUnavailableSeatRegistryAsWarning`, `TestDoctorSortsDuplicateSeatLoginAdvisoriesByEmail`) |
...
| database user-version read | JAIL | cannot read is a hard failure, rc 3 | `internal/doctor/doctor.go` |
| database quick-check read | JAIL | cannot read is a hard failure, rc 3 | `internal/doctor/doctor.go` |
| database schema or integrity mismatch | JAIL | mismatch is a warning and rc 1 | `internal/doctor/doctor.go` |
| shared-store health | JAIL | healthy is clean; degraded shared state is a warning and rc 1 | `internal/doctor/doctor.go` |
| row counts and orphaned hides | JAIL | count query failure is a hard failure, rc 3; nonzero orphaned hides is a warning and rc 1 | `internal/doctor/doctor.go`, `store/hidden_test.go` |
| WAL stat | JAIL | absent WAL is clean; other stat failure is a warning and rc 1 | `internal/doctor/doctor.go` |
| busy-warning counters | JAIL | zero counters are clean; nonzero counters are a warning, rc 1; an unreadable counter is a hard failure, rc 3 | `internal/doctor/doctor.go` |
| process-table read | JAIL | readable, including empty, is clean; unreadable is a warning and rc 1 | `internal/doctor/doctor.go`, `internal/gather` |
| configured roots | JAIL | existing directories are clean; missing, non-directory, or unreadable roots are warnings and rc 1 | `internal/doctor/doctor.go` |
| SID crumb directory | JAIL | missing after clean install is empty and clean; readable invalid entries warn; non-directory or unreadable probe remains an error and rc 1 | `internal/doctor/jail_test.go`, `main_test.go` |
| harvestpy plan | JAIL | available unblocked plan is clean; unavailable or blocked exact lock is a warning and rc 1 | `internal/doctor/harvest_test.go` |
| harvestpy deliberate skip | JAIL | deliberate skip is reported as `skipped` and is not a warning | `internal/doctor/harvest_test.go` |
| harvestpy interpreter and marker | JAIL | healthy interpreter and readable marker are clean; failed execution or marker inspection is a warning and rc 1 | `internal/doctor/harvest_test.go` |
| harvestpy lock and inventory | JAIL | complete lock and inventory are clean; incomplete or unreadable state is a warning and rc 1 | `internal/doctor/harvest_test.go` |
| harvestpy live smoke and conversion | JAIL | both healthy are clean; either failure is a warning and rc 1 | `internal/doctor/harvest_test.go` |
| harvester cache | JAIL | missing cache root is clean; walk failure is a warning and rc 1; the root is `cache.dir` or `<home>/.professor/.cache`, never the working directory or a retired env variable | `internal/doctor/doctor.go`, `internal/harvest/legacy_core_parity_test.go` (`TestCacheRootIsConfiguredDirOrTheOneDefault`), `internal/harvestmcp/service_test.go` (`TestServiceCacheIsTheOneRootNotTheWorkingDirectory`) |
...
| spawn-audit classifier: injected, predates-layer, and violation verdicts | UNIT | `--system-prompt-file` or the lean env arm classifies INJECTED only when argv ALSO carries `--settings {"outputStyle":"default"}` (or that object plus a merged `"theme"` key, since `argvCarriesOutputStyleDefault` parses the JSON rather than byte-comparing it) — carrying the prompt without it is a VIOLATION only when the process started AT/AFTER the staged prompt layer's mtime; a seat born BEFORE the stamp is PREDATES-LAYER instead (a reload fixes it, not a bug hunt), while an unusable age signal (no birth time, no stamp) or a correctly-flagged seat is never used to excuse or downgrade the verdict; an unreadable environment never clears a flagless seat | `internal/doctor/spawn_audit_test.go` (`TestClassifySpawnSeparatesInjectedOldAndBypassed`, `TestArgvCarriesOutputStyleDefaultAcceptsAThemedPayload`) |
...
| --- | --- | --- | --- |
| baseline doctor, run before any owned binary is replaced | JAIL | records `(warnings, failures)`; a baseline that cannot run, or exits 2 (usage), never blocks — printed and treated as "no baseline" | `internal/update/run_test.go` (`stubUpdateBaselineDoctor`, every `TestUpdate*` test that drives a real `Run()`) |
...
| candidate doctor exit 2 or a spawn error | JAIL | rolls back — a doctor that cannot run is not a verdict | `internal/update/run.go` (`runUpdateDoctor`/`updateRepository` gate switch) |
| candidate git/build/install/doctor process seam | UNIT | update consumes the injected `deps.Runner`; candidate stdout/stderr remain live, and doctor output is also captured for its typed exit verdict | `internal/update/runner_test.go` (`TestRunUpdateCandidateCommandStreamsAndCapturesDoctor`) |
...
| behavior | safety | expected | regression |
| --- | --- | --- | --- |
| post-install candidate doctor reads the migrated config path, not the stale pre-install one | JAIL | `--config` names `pfm.config.json`; stdout carries `config migrated by the update: <old> → <new>` | `internal/update/files_test.go` (`TestUpdateCandidateDoctorReceivesTheMigratedConfigPath`) |
...
| a rollback residue left by a concurrent settings edit names the stranded unknown hook commands it carries | JAIL | stderr contains `it still carries` and the stranded subcommand name | `internal/update/hook_residue_test.go` (`TestUpdateRollbackResidueNamesTheStrandedHookCommands`) |
...
| `pfm doctor`'s `mcp client=claude` row names every registry `ClaudeUserRegistries` resolves, with its reason and both the `harvester` and `chat` states, and warns with a remediation on a registry a sibling registry's `pfm` registration shows was never reached | JAIL | two rows: the healthy implicit-account registry (`harvester=pfm chat=pfm`, no warning) and the absent ambient registry (`harvester=absent chat=absent remediation=run pfm install --yes (registers every registry a pfm-launched Claude reads)`) | `internal/doctor/mcp_client_test.go` (`TestDoctorMCPClientRowNamesEachRegistryAndItsReason`) |
...
pfm's `claude` launcher (`assets/bin/claude`) never symlinks the canonical binary straight into `~/.local/share/claude/versions/`, so Claude Code's own updater reports version cleanup disabled and every build it ever installs accumulates. The launcher asset delegates to `pfm internal claude-launch`; `ClaudeLaunch` calls `ResolveClaudeBinary`, which calls `InspectClaudeVersions` for its native-version fallback. `internal/installer/claude_versions.go` splits the work in two: `InspectClaudeVersions` (enumeration, ordering, and the structural protections — newest, unparsed name, configured `claude.binary`, displaced native target) never touches a process table at all, because the launcher's hot path only needs the selected binary; `ProbeLiveClaudeVersions` is the separate, deliberately optional live-process half only `pfm doctor` and pruneClaudeVersions call. That probe scopes candidates the same way `internal/stale.Find` does (stale.go:53-89): a pid is worth `Image`'s cost only when its argv[0] names the Claude binary, lives inside the versions directory, or its own basename parses as a version — anything else, and any Cmdline read that fails, is skipped before `Image` is ever called, and a candidate's `Image` failure is refused (named "probe failed") UNLESS the pid has already exited (`kill(pid,0) -> ESRCH`), which is skipped instead. `PlanClaudeVersionPrune` decides what a prune may remove: everything outside the keep window and not otherwise protected, or nothing at all when the live probe could not vouch for every candidate.

| behavior | safety | expected | regression |
| --- | --- | --- | --- |
| the managed launcher asset delegates every invocation to `pfm internal claude-launch`; `ClaudeLaunch` calls `ResolveClaudeBinary`, whose native-version fallback calls `InspectClaudeVersions` and selects the highest parsed version before PATH | JAIL+sh | configured binary wins, then the highest parsed native version, then PATH; the exact launcher asset carries no duplicate selection logic | `internal/installer/launcher_test.go` (`TestClaudeLauncherAssetIsExactExecShim`, `TestResolveClaudeBinaryUsesConfiguredThenNewestThenPATH`), `internal/installer/claude_versions_test.go` (`TestInspectClaudeVersionsMarksLiveNewestConfiguredAndUnparsed`) |
| compatibility command `pfm internal claude-version` never constructs or reads a process table at all | JAIL (structural) | `internal/hookentry/claude_version.go` names no `gather.*` symbol | `cmd/pfm/internal_claude_version_test.go` (`TestInternalClaudeVersionCommandNeverConstructsAProcessTable`) |
...
| `pfm doctor` reports `claude-versions dir=… count=N bytes=… newest=… live=…(n pids) prunable=N (…)`, warns only when something is prunable or the probe failed, and never warns when the native installer simply never ran (dir absent) | JAIL | a 3-version fixture with one live (1 pid) and one prunable reports `count=3` and `prunable=1`; `PFM_PROC_ROOT` fixture supplies the live pid's `exe` AND `cmdline` (candidate scoping reads argv[0] before `Image`) | `internal/doctor/jail_test.go` (`TestDoctorReportsClaudeVersionCountBytesAndPrunable`) |
| `pfm internal claude-version` prints the newest parsed version's absolute path and exits 0, or prints nothing and exits 127 when there is none to choose | JAIL | a fixture with the higher version at the older mtime still prints the higher version's path; an empty `versions/` directory exits 127 with empty stdout | `cmd/pfm/internal_claude_version_test.go` (`TestInternalClaudeVersionPrintsNewestOrExits127`) |
...
| picker/name-sync live scans seed empty Codex pane bindings but never overwrite a hook-supplied post-clear id | JAIL | `cmd/pfm/internal_clear_kill_jail_test.go`, `internal/picker/pipeline.go` | |
...
| an UNREADABLE kill table never drops a binding, and an explicit `pfm chat kill` (no prompt baseline) never does either | JAIL | `cmd/pfm/codexpanes_test.go`, `internal/picker/pipeline.go` | |
| `pfm doctor` names contested and clear-retired pane bindings, and an unreadable binding table is not an empty one | JAIL | `internal/doctor/doctor.go`, `cmd/pfm/doctor_codex_pane_test.go` | |
| `pfm doctor` counts a binding whose PANE is gone as stale, never contested — a real host held 76 bindings for 19 live panes, and counting the dead ones reported 8 false emergencies | JAIL+tmux | `internal/doctor/doctor.go`, `cmd/pfm/doctor_codex_pane_test.go` | found by running the new check on the live fleet |
| `pfm doctor` names a live Codex pane pfm cannot follow through a clear; the reconcile pass stays quiet there so the picker is not spammed | JAIL+tmux | `internal/doctor/doctor.go`, `cmd/pfm/doctor_codex_pane_test.go`, `internal/picker/refresh_gather_warnings_jail_test.go` | |
...
| `pfm chat reload` takes `--account N`; a bare prose word is refused with the flag it should have been | JAIL | `cmd/pfm/chat_reload_command.go`, `cmd/pfm/chat_reload_args_test.go` | |
...
| `Esc` after `⌃X` keeps the applied kill receipt but abandons a pending account change | JAIL | `ui/model_test.go`, `internal/picker/picker.go` | **B3** |
...
| `⌃S` cycles primary account through the configured account-id roster | JAIL | `ui/model.go`, `internal/picker/pipeline_test.go` | |
| `⌃S` change persisted through the shared store on exit | JAIL | `internal/picker/picker.go`, `internal/picker/pipeline.go` | |
| `⌃O` reboot a live row → kill-server, drop crumbs, demote to resume kind | JAIL+tmux | `ui/model.go`, `internal/picker/picker.go` | |
| `⌃O` on a non-live row → no-op | JAIL | `ui/model.go:167-171` | |
| `⌃O` on a `LiveSplit` (no id) → error, never a half-reboot | JAIL | `internal/picker/picker.go` | |
| `Enter` → `OutcomeSelected` with the row; empty filter → no-op | JAIL | `ui/model.go:173-179` | |
| `Esc` / `⌃C` → cancelled, rc 0, no writes | JAIL | `ui/model.go`, `internal/picker/picker.go` | |
...
| a PARKED stream's Codex idle-identity poll runs `/proc/<pid>/fd` (FDLinks) every 10s (widened from 2s) but pays for the tmux `capture-pane` half of reconciliation ONLY when the FDLinks-observed rollout set changed for some PID since the previous poll — a held rollout or an identity-error PID that is unchanged is skipped outright; a rollout-LESS PID (no open rollout fd — the normal shape of a paginated Codex thread since 0.146.1) is never treated as skippable, since procfs has no opinion for it at all (2026-09-08: ~4 capture-pane execs per 5s measured on an idle Limits tab) | JAIL+tmux | `internal/picker/pipeline.go` (`codexRolloutFingerprints`, `codexRolloutFingerprintsEqual`, `codexRolloutFingerprintsSkippable`), `internal/picker/pipeline_test.go`, `internal/picker/refresh_codex_clear_retry_jail_test.go` (`TestParkedPickerStillObservesCodexClear`, `TestParkedPickerRetriesRefreshAfterCodexClear`) | |
...
| **continued claude chat** — Claude moved the chat to a background job under a new session (`continued-in` record) → ONE row under the successor's id and name, whether a pane crumb or agent names the old segment or nothing runs; the superseded segment is never a resume row in either frame; a successor never indexed leaves the old segment standing | JAIL | `fleet/continuation.go`, `fleet/continuation_test.go`, `index/continuation_test.go`, `store/continuation_test.go` | |
...
| **agent row** → live claude under a NON-primary `CLAUDE_CONFIG_DIR` carrying `--session-id`/`--resume`, or a Claude daemon job on ANY config dir (`claude bg-spare`, session read from its own `<config>/sessions/<pid>.json`: same pid, `procStart`, `kind:"bg"`); an unreadable record is a snapshot warning, never an absence | JAIL | `gather/agents.go`, `gather/agents_bgjob_test.go` | B3 |
...
| **vsct bunker chat** → `vsct` sockets excluded from the probe; open uses `exec` | JAIL+tmux | `gather/tmuxprobe.go`, `internal/picker/pipeline.go`, `action/synth.go` | |
...
| **both accounts** — a row's account comes from the longest matching config root | JAIL | `compose/compose.go`, `internal/picker/pipeline.go` | |
| **primary switch** — db meta first, `~/.claude-primary` mirror second, off-roster → 1 | JAIL | `fleetdb/fleetdb.go`, `internal/picker/pipeline.go` | |
...
| Hiding a LIVE chat (resolved socket+pane) also runs the `--exit` finisher — CLI `chat kill <id\|self>`, picker ⌃X, and MCP `chat_kill` all resolve the address and hit the same `kill.Manager.Kill` branch; a resumable-only target stays a store write | JAIL+tmux | `kill/manager.go`, `internal/picker/pipeline.go` (`resolveRowTarget`), `ui/model.go` (`toggleKilled`), `mcpserv/actions.go` (`chatKill`) | |
| `pfm chat reload --new --hide` never runs the finisher on the conversation it hides — the pane it left behind now belongs to the reborn chat, so `hideReloadedConversation` calls `kill.Manager.Kill` with no socket/pane | JAIL+tmux | `cmd/pfm/chat_reload_command.go` (`hideReloadedConversation`), `cmd/pfm/chat_reload_hide_test.go` | |
...
| Every Claude launch (interactive, resume, probe, query, the launcher shim, headless `pfm headless exec`/`pfm ask`) carries `--settings {"outputStyle":"default"}` (merged with a resolved `claude.theme` under a `"theme"` key when the launching account has one set) so a project/user `outputStyle` never double-applies over the staged professor prompt; Codex and OpenCode carry nothing of it | JAIL | `internal/engine/builtin.go` (`Claude.LaunchArgs`, `ClaudeSettingsPayload`), `internal/action/claude_spawn.go`, `internal/headless/run/run.go` (`arguments`) | `internal/action/output_style_settings_test.go`, `internal/action/theme_test.go`, `internal/headless/run/output_style_settings_test.go`, `internal/headless/run/theme_test.go` |
...
| Bunker (`$TMUX` socket basename `vsct`) → `exec` prefix on every launch line | JAIL | `action/synth.go`, `internal/picker/pipeline.go` | |
...
| `chat_ls` primary account agrees with the picker | JAIL+sh | `mcpserv/backend.go` vs `internal/picker/pipeline.go` | |
...
| Flow | Safety | Source |
| --- | --- | --- |
| Missing/non-executable PFM stops shim sourcing with a diagnostic | JAIL+sh | `internal/installer/shim/shim_test.go` |
| Source/re-source leaves no public `cc*` functions or aliases; external compiler remains | JAIL+sh | `internal/installer/shim/shim_test.go` |
...
| Captured output stays a shell line; terminal output executes the action | JAIL | `internal/action/dispatch_test.go` |
...
| flow | safety | expected behavior (source) | regression |
| ------------------------------------------------------------------------------------------------------------------ | ---------------- | ----------------------------------------------------------------------------------------------------- | ---------- |
| shared-store open is idempotent, initializes schema, enables WAL and sets a busy timeout | JAIL | `internal/fleetdb/fleetdb.go`, `internal/fleetdb/fleetdb_test.go` | |
| kill add/delete/lookup use SQLite exclusively and never recreate the retired carrier | JAIL | `internal/fleetdb/fleetdb.go`, `internal/fleetdb/fleetdb_test.go` | B2 |
| clear-kill baselines survive reads, expire after transcript growth, and cannot weaken a permanent kill | JAIL | `internal/fleetdb/fleetdb_test.go`, `internal/store/killed_test.go`, `internal/compose/compose_test.go` | |
| kill sync and orphan pruning replace only the intended set, in one transaction | JAIL | `internal/fleetdb/fleetdb.go`, `internal/store/killed_test.go` | |
...
| hidden `internal primary-get/set` validates the 1/2 roster and keeps its mirror coherent | JAIL | `cmd/pfm/main.go`, `internal/fleetdb/fleetdb_test.go`, `internal/installer/shim/shim_test.go` | |
| child add/list/clear distinguishes `new` servers from `pane` children | JAIL | `internal/fleetdb/fleetdb.go`, `internal/fleetdb/fleetdb_test.go` | |
...
| `chat reload` parses account/cache-only forms and rejects a multi-pane server without an explicit pane | JAIL+tmux | `chat_reload_args_test.go:TestReloadAccountFlag`, `chat_reload_jail_test.go:TestChatReloadAcceptsCacheOnlyRequest`, `chat_reload_command_test.go:TestReloadTargetWithSockOnlyKeepsTheSinglePaneRule` | |
...
| `chat reload` serializes by pane, refuses an open selector, and waits for a real prompt before `--then` | JAIL+tmux | `internal/reload`, `chat_reload_jail_test.go:TestChatReloadRefusesAnOpenSelectorOnAProbeSocket`, `internal/reload/reload_test.go` | |
...
| the scheduler hands its detached `reload-run` worker an explicit `--sock`/`--pane` it already resolved (ambiently or from the caller's own `--sock`), so the worker never re-derives identity | JAIL+tmux | `chat_reload_command.go:runChatReloadWithRuntime`, `chat_reload_jail_test.go:TestChatReloadHandsTheWorkerAnExplicitSockAndPane` | |
| `reloadTarget` with `--sock`+`--pane` selects the named pane out of a multi-pane server directly (never falls through to `resolve.NewWhoami`), and reports the pane by name when it is no longer live | JAIL | `chat_reload_command.go:reloadTarget`, `chat_reload_command_test.go:TestReloadTargetWithPaneSelectsItAmongSeveralWithoutWhoami`, `TestReloadTargetWithPaneRejectsAPaneThatIsNotLive` | |
| `reloadTarget` with `--sock` alone (no `--pane`) keeps the pre-existing single-pane rule: one pane auto-selects, more than one is refused | JAIL | `chat_reload_command.go:reloadTarget`, `chat_reload_command_test.go:TestReloadTargetWithSockOnlyKeepsTheSinglePaneRule` | |
...
| install dependency preflight uses the doctor registry; missing required deps refuse before installer writes | JAIL | `cmd/pfm/install_preflight_test.go`, `internal/deps/probe_test.go` | |
...
| install retires every legacy Dream hook copy and preserves unrelated same-event neighbors | JAIL | `TestDreamHookPauseRetiresEveryCopyAndPreservesNeighbors` | |
| uninstall removes only ledger-owned hook occurrences and preserves operator-owned neighbors | JAIL | `TestDreamHookPauseRetiresEveryCopyAndPreservesNeighbors` | |
...
| `pfm doctor` fails (not warns) when either host-overlay symlink is missing/displaced, or a configured account's `statusLine.command` still names the raw `pfm statusline` instead of the overlay | JAIL | `internal/doctor/host_overlay.go` (`printHostOverlayDoctor`), `internal/doctor/host_overlay_test.go` | issue #14 F1 |
...
| 6 | **Live-vs-indexed refresh race in the picker** (`internal/picker/pipeline.go`, `ui/model.go`) | Three snapshots stream into an open TUI while the user is toggling kills. A row that changes identity between frames takes the pending kill with it. |
...
| `internal/deps` | `Runner` (`Run(ctx, argv, opts) (RunResult, error)`, `LookPath`, `Start(ctx, argv, opts) (Process, error)` for a streaming or detached spawn; `StartOptions` owns ordinary stdin, stdin/stdout pipes, and optional process groups) | `deps.RealRunner` — `exec.CommandContext`; `Start(Detach: true)` runs `SysProcAttr{Setsid: true}` and releases the process right after Start; `KillGroup` targets the requested process group | `deps.FakeRunner` — scripted by argv prefix (longest match wins), `Calls()`/`Starts()` plus `LifecycleCalls()` ledgers, `ScriptInteractive` owns deterministic pipes, and an unscripted streaming `Start` returns `UnscriptedError` |
| B4 harvestpy worker cancellation | converter and browser workers own bidirectional Runner pipes, close stdin before kill/wait, and report cleanup failures; blocked writes and reads observe request cancellation | `harvestpy.Converter` / `BrowserWorker` with `deps.RealRunner` and process groups | `TestConverterCancellationReleasesBlockedWrite`, `TestBrowserWorkerCancellationClosesPendingRead`, `TestAskProtocolRoundTrip`, `TestNilAskHandlerFailsClosed`, `TestRealRunnerStartOwnsInteractivePipesAndProcessGroup`, `TestFakeRunnerInteractiveProcessCanReleaseBlockedWrite` |
...
| B5a consumers (`doctor`, `installer`, `update`, `updatecheck`, `usagehook`, `mcpserv`, `binwatch`) | existing `Clock`, `Env`, `Runner`, and listener seams at package boundaries | production defaults (`clock.Real`, `paths.OSEnv`, `deps.RealRunner`, `net.Listen`) | package tests inject `clock.Fake`, `paths.MapEnv`, `deps.FakeRunner`/scripted Start, and listener functions; no behavior path constructs a second real Runner |
| B5b consumers (`action`, `archive`, `chat`, `config`, `fleet`, `headless`, `heal`, `nudge`, `picker`, `professor`, `rearm`, `resolve`, `store`) | `clock.Clock`, `paths.Env`, `deps.Runner`, and `atomicfile.Write` at package boundaries; direct Claude and headless process calls preserve streaming, stdin, cancellation, and process-group behavior | production defaults (`clock.Real`, `paths.OSEnv`, `deps.RealRunner`, `atomicfile`) | `action.TestClaudeSpawnCommandUsesRunnerStartBoundary`, `headless/run.TestRunUsesScriptedRunnerStartBoundary`, `headless/run.TestRunProcessBoundsWaitAfterCancellation`, and `professor.TestStoreSHAWithRunnerUsesScriptedGit`; remaining package suites cover the existing clock/env/atomic behavior. |
...
| 4 | `NoTmux` / `OldTmux` | `deps.FakeRunner` returns ENOENT for tmux / a version below `deps.Registry`'s minimum | `tmux`, `doctor`, `spawn`, `reload`, B5a doctor Runner probes |
| 5 | `NoServiceManager` | ENOENT for `systemctl` and `launchctl` | `installer` units, `doctor`, `kill` service scope, B5a installer Runner paths |
...
| 8 | `ExpiredCreds` / `NoCreds` | `.credentials.json` with `expiresAt` in the past; the file absent plus a Keychain (`security`) not-found script | `usagehook`, `doctor` seat rows and duplicate-login registry advisory, `headless`, `resolve` |
