...
| `pfm codex agents [--home PATH]` — no positional | JAIL | compiles `{home}/.professor/templates/global/agents/*.md` into the pfm-owned generated directory (`paths.GeneratedCodexAgentsDir`, never beside the source), installs sources into `{home}/.claude/agents` and the generated TOMLs into `{home}/.codex/agents`; a link still pointing at the retired in-clone twin re-points on the next run; escaping mirrors build-codex.mjs:151-153 byte-for-byte (verified against the retired host `build-global-agents.py`, since deleted) | `internal/codexgen/globalagents_test.go`, `cmd/pfm/codex_agents_command_test.go` |
