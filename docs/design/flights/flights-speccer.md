...
- Exact text appears only where `flights-speccer` copied it from the code or saw a command print it during the run. A command, a flag, an exit code or an output it has not observed is written as intent. A shape or a literal written from memory is a defect: precision that was never run reads as verified.
...
- On `SPEC-DRIFT` the dispatcher starts nothing that needs that task, sends the report, what already landed and the completed ids back to the same `flights-speccer`, and dispatches again from the rewritten index.
