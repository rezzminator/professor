...
| --- | --- | --- | --- |
| `calls` | tool call | `tool_use_id` | `session_id`, `agent_id`, `agent_type`, `request_id`, `ts`, `tool`, `input` (see below), `cwd`, `duration_ms`, `failed`, `error`, `bytes_real`, `bytes_delivered`, `persisted_path`, `file_path`, `file_bytes`, `file_bytes_before`, `read_start`, `read_lines`, `read_total_lines`, `source`, `config_dir`, `account`, `seat_dir` |
| `requests` | model request | `request_id` | `session_id`, `agent_id`, `ts`, `context_tokens`, `output_tokens`, `calls`, `pending`, `source`, `config_dir`, `account`, `seat_dir` |
| `agents` | sub-agent | `agent_id` | `session_id`, `agent_type`, `parent_tool_use_id`, `started`, `stopped`, `transcript_path`, `total_tokens`, `tool_uses`, `model`, `source`, `config_dir`, `account`, `seat_dir` |
...
- `config_dir` is the history home, not who ran the call; `seat_dir` and `account` are. `seat_dir` is the hook process's `CLAUDE_CONFIG_DIR` — the account dir the chat was launched with — absolute and clean with symlinks NOT resolved, so `~/.cc/3` stays `~/.cc/3`; unset or blank, it is `{home}/.claude`. `account` is the id of the machine config's account whose `configDir` (with `~` expanded) is that dir, both sides compared with symlinks resolved, or cleaned when either cannot be resolved; NULL when no account names it. The hook loads the machine config once per run, the file `pfm callmeter` reads (the current config file, else the legacy one beside it), through the config package's loader. A config that cannot be loaded, or is absent, leaves `account` NULL, still records `seat_dir`, and writes one `account` fault for the run. Both are set on every call, request and agent the hook writes, and merge like `config_dir`: a set value is never overwritten by a write that lacks it, and a provisional request's pair survives its resolution.
...
- `faults.stage` is one of `payload`, `store`, `transcript`, `parse`, `account` (the machine config could not be loaded, so the run's account is unknown; one per hook run), naming where the failure happened. A fault that cannot itself be written to the store (the store is unreachable) goes to pfm's log only.
...
`pfm callmeter report {topic} [--since D] [--project P] [--agent-type T] [--session S] [--config-dir DIR] [--account N] [--limit N]`, each topic a fixed query over the store. Every report prunes rows older than 30 days before it runs; the hook never prunes.
...
| `--account N` | narrows calls and requests to the ones that configured account id ran, and the header names `account=N`; rows with no account recorded are left out. Default: every row, those with no account too |
...
The command surface (`pfm/internal/callmeter/command/`, a thin dispatch in `pfm/cmd/pfm/main.go`): with no store it prints `callmeter: no store at {path}: nothing recorded yet`, exits 0 and creates no file; a store that cannot be opened exits 1; a bad topic, action or flag, or a `--config-dir` or `--account` the machine config does not name, exits 2 naming the configured dirs or ids; a `--since` older than 30 days is clamped with one stderr note; `--project` is made absolute, symlinks unresolved.
