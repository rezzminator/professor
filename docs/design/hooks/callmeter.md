...
`$HOME/.local/state/pfm/callmeter.db`, its own file: every call of every chat writes to it, and it must never contend with `fleet.db`. WAL mode, `busy_timeout` 5 s against the concurrent async writers, one short transaction per event. `PRAGMA user_version` carries the schema version (1 for this design); a store written by a newer schema is refused with an error naming both the store's version and the binary's, never opened and misread.

Every write is an upsert by its row's key, setting only the columns its event owns and leaving the rest untouched, because async hooks and backfill arrive in any order — a `PostToolBatch` can land before its `PostToolUse`, and backfill can land after either. `ts` is an integer Unix timestamp in milliseconds, UTC, in every table that carries one.
...
- `source` is `hook` or `transcript`: which path wrote (or last touched) the row. `config_dir` is the config dir whose `projects/` physically holds the row's transcript (`callmeter.ProjectsHome`: the `projects/` directory resolved through symlinks, then its parent). Accounts that share one `projects/` — on this machine `~/.claude2/projects` and `~/.claude3/projects` are symlinks to `~/.claude/projects` — hold one file per chat, so they are one config dir: a chat keeps one history and one set of metrics whichever account runs it, the hook and backfill name the same dir, backfill reads each shared transcript once, and `--config-dir` with any of those accounts shows the same history. Accounts with separate `projects/` stay separate.
...
- `faults.stage` is one of `payload`, `store`, `transcript`, `parse`, `backfill`, naming where the failure happened. A fault that cannot itself be written to the store (the store is unreachable) goes to pfm's log only.
...
- Retention: rows older than 30 days are pruned before `pfm callmeter backfill` and before every `pfm callmeter report` run; the hook itself never prunes, so a write always succeeds even mid-prune-cycle. Backfill skips transcript entries older than the 30-day window, so it never re-adds what a prune already removed.

## Parsing a command

A Bash call is often several commands (`F=x; wc -l "$F" && grep -n func "$F"`) and often carries another language inside it (`python3 -c "…"`, `python3 - <<'EOF' … EOF`, `node -e "…"`). The parse happens at report time only (`report.EnsureParsed`), never in the hook and never in backfill, and its result is cached in `command_parts`, keyed by the call. No parser is written here; each language uses its established one:
...
`pfm callmeter report {topic} [--since D] [--project P] [--agent-type T] [--session S] [--config-dir DIR] [--limit N]`, each topic a fixed query over the store. Every report prunes rows older than 30 days before it runs, the same as backfill.
...
The command surface (`pfm/internal/callmeter/command/`, a thin dispatch in `pfm/cmd/pfm/main.go`): with no store it prints `callmeter: no store at {path}: nothing recorded yet`, exits 0 and creates no file; a store that cannot be opened exits 1; a bad topic, action or flag, or a `--config-dir` the machine config does not name, exits 2 with usage naming the configured dirs; a `--since` older than 30 days is clamped with one stderr note, for report and backfill alike; `--project` is made absolute, symlinks unresolved.

## Backfill

`pfm callmeter backfill [--since D] [--config-dir DIR]` builds the same rows from transcripts already on disk: `projects/**/*.jsonl` and each session's `subagents/`, across every Claude config dir the machine config names, or just the one `--config-dir` narrows to. A transcript holds every tool call, the text delivered to the model, the model message that grouped the calls and its `usage`. It does not hold a file's size at read time or before a write: those columns stay empty for backfilled rows, marked `source = transcript`. A sub-agent's totals are its own transcript's sums by the hook's `SubagentStop` rule (`callmeter.ReadAgentTotals`), written after every transcript of the run over the parent's `Agent` result, whose model alone stays, and only into a row the run wrote, so an agent wholly outside the window adds nothing; they are the one value backfill overwrites, so a hook row summed at an early stop heals to the whole transcript. An agent that never replied sums to 0, never unknown. Its `stopped` fills, never over a hook's, with the timestamp of the transcript's last assistant entry when that entry ends the turn (`AgentTotals.Final`); a transcript still running or killed mid-turn leaves it empty. Claude Code keeps every agent of a session in one flat `subagents/` directory however deep the spawn, so the child of a sub-agent is `subagents/agent-{child}.jsonl` beside its parent (`childTranscript`).

Backfill is an upsert by key: it inserts rows missing entirely, and for rows the hook already wrote it fills only the columns still empty — it never overwrites a value the hook recorded. One hook value it replaces: a call still keyed to a provisional `pending:{tool_use_id}` request whose message the transcript holds is resolved as the hook's `resolvePending` does (`Tx.ResolveRequest`), before the message row is filled, so the provisional row merges into the message row and no pending row is left behind. It prunes rows older than 30 days before it runs, and it skips transcript entries older than that window, so it never re-adds a row a prune already removed. A transcript it cannot read is a `backfill` fault and one named line in its summary, never skipped silently. Its summary prints transcripts read, transcripts unreadable, and calls inserted and filled.

## Corpus findings and fixes

A backfill of 30 days of real transcripts (62,005 calls, 4,068 transcripts, three accounts) parsed all 37,355 Bash calls, and exposed where attribution was wrong. Each rule below answers one measured defect:
...
- **Malformed calls are calls.** A transcript `tool_use` whose input the harness rejected (a Read with no `file_path`, an `offset` given as an array) is recorded as a failed call with the error `malformed input: {cause}`, never dropped.
...
Unit tests alone passed while this corpus failed, so two corpora made of real data now guard the parser and backfill:

1. **Real commands.** A table-driven test of Bash commands taken from the transcript backfill (nested quoting, heredocs, `cd` chains, wrappers, globs, redirects, inner `bash -c`, Python heredocs, loops, arrays), each with its expected parts, attributions, actions and `conditional` flags. Paths are rewritten to `/tmp/demo-proj/…`; the case keeps the command's structure verbatim. The corpus lives in `pfm/internal/callmeter/cmdparse/testdata/commands-corpus.json`; a case whose parse differs from the spec carries `known_defect` and is skipped by name until it matches.
2. **Real long transcripts.** Two or three of the longest, most varied transcripts from this repository's own sessions (a main chat with sub-agents), passed through a checked-in sanitizer that rewrites every path, replaces all prose and every tool output with filler of the same byte length, and keeps every `tool_use` input's structure. Backfill runs over them in a test that asserts the call, request and agent counts, the parse status per language, the attributions of named calls and the byte sums. The fixtures live in `pfm/internal/callmeter/report/testdata/corpus/`, and the test in `report/corpus_test.go` moves their roots under a temp dir and creates the few files whose existence it asserts.
...
- File columns (`FileColumnsFromInput`, `FileColumnsFromResult`), shared by the hook and backfill: `Read`, `Write`, `Edit`, `MultiEdit`, `NotebookEdit`; a relative path joins `cwd`; the Read input's `offset`/`limit` are overridden field by field by the result's `startLine`/`numLines`/`totalLines`; `type: create` with a null `originalFile` is 0; a file tool with no path is a fault.
- Backfill (`backfill/backfill.go`): a request's call count is its `tool_use` blocks sharing one message id; a `tool_result` outside the window is ignored; a provisional request id the hook left is resolved to the message id the transcript names (`storedPending`, then `Tx.ResolveRequest`), its merged `source` staying the hook's; faults are written after each transcript's transaction commits; a `tool_use` whose input `FileColumnsFromInput` cannot read is a call with `failed = 1` and `error = malformed input: {cause}` that its `tool_result` never overwrites, not a malformed line; the summary separates unreadable transcripts, malformed lines and config dirs with no `projects/`.
...
- Corpus sanitizer (`corpussan/`, run by `go -C pfm run ./internal/callmeter/corpussan/cmd/corpussan`): strings under id/enum keys stay when they are short tokens; paths, commands and patterns are rewritten (project root → `/tmp/demo-proj`, every other absolute root → `/tmp/demo-home/…`, encoded project dir names too), and every word in them that is not a number, ≤2 characters, a built-in program/keyword, a name under `-tree`, or a flag becomes same-length `x`; every other string becomes `x` filler of the same byte length with newlines kept; ids are kept verbatim; the home dir's components and `-deny` words never survive; a line that is not JSON is an error naming file and line; `tool-results/` is not copied; output never overwrites a file.
- Found live, first minutes after install (HOUSING and this repo's chats): a failed Bash call's size is its error text, the hook's `PostToolUseFailure` `error` and backfill's `tool_result` content alike (the transcript's `toolUseResult` is the same text behind `Error: `); Claude Code stops its own internal agents with a `SubagentStop` that has no `agent_type`, no `SubagentStart` and no transcript on disk, and the hook records nothing for those (16 false transcript faults before), while a typed agent missing its transcript stays a fault; the same exemption covers `PostToolBatch` for one of these agents, which used to fail `FindRequests` against the missing file, write a pending request, then fail `resolvePending` against the same file for a second fault; a call still running has only its `PreToolUse` row until its result lands; a headless `claude -p` cancels running hooks at exit ("Hook cancelled"), so its last calls can lose their `PostToolUse`, and backfill fills their input and sizes from the transcript. Running chats pick up the new hooks without a restart.
...
- Backfill heals what the hook left (`backfill.go`): 15 requests stayed `pending:` after their agents stopped with the hook absent, every `tool_use` id already on disk, and 2,195 backfilled agents had no `stopped`, since backfill never set it. Backfill now resolves a pending request whose message it reads, and fills `stopped` from a transcript whose last message ends the turn — and, since many finished sub-agent transcripts carry `stop_reason: null`, from a task notice in the parent transcript naming that agent completed, failed or killed, most of them (live: 114 of 127) only ever an enqueued `"queue-operation"`, never a delivered "user" turn. An agent with neither a notice nor a finished final entry keeps no stop.
- Backfill also retires what no transcript can ever resolve (`backfill/orphans.go`): rows the hook wrote before the `untypedAgentMissingTranscript` exemption existed (hookentry/callmeter.go) are Claude Code's own internal sub-agents — `agent_type` NULL, no `agents` row, no `subagents/agent-{id}.jsonl` ever written — so `storedPending` never finds their message and they stay `pending:` forever. A pass after every transcript walk deletes a pending request (and its calls) when it is older than a 10-minute grace for the hook's own resolve, its `agent_id` is non-empty, no call of it carries a typed `agent_type` and no `agents` row names that agent_id, and its sub-agent transcript is absent (`fs.ErrNotExist`) under every configured config dir's `projects/`; a transcript that exists, a typed agent, a main-chat request (`agent_id` empty) or a row still inside the grace is left alone, and any other stat failure is a fault, never read as absence. The count lands on the summary's `retired N`.
...
- No Codex or OpenCode recording in the first version. Their session files are a later backfill source.
- No calls the harness refused before running (an unavailable tool): they leave no hook event, and the backfill counts them from the transcript only.
...
| The store and reports | `pfm/internal/callmeter/` and its `cmdparse/`, `backfill/`, `report/`, `command/` packages; `pfm/cmd/pfm/main.go` | schema, parser, `report`, `backfill`, the CLI |
...
- Retention: **Decided.** 30 days. `pfm callmeter backfill` and every `pfm callmeter report` prune rows older than 30 days before they run; the hook never prunes. Backfill skips transcript entries older than the window, so it never re-adds what a prune removed.
- Whether reports read the other accounts' config dirs by default or only the current one: **Decided.** Reports and backfill cover every Claude config dir the machine config names by default; `--config-dir DIR` narrows to one.
