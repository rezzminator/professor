...
// test can drive the gateway's browser rung. It records every call in order.
type browserConverter struct {
	fakeConverter
	mu    sync.Mutex
	calls []string // one entry per FetchBrowser call: the source it was given.
	reply func() (string, int, error)
}

// FetchBrowser answers with reply, the render landing on the page requested.
func (b *browserConverter) FetchBrowser(_ context.Context, source string) (string, int, string, error) {
	b.mu.Lock()
	...
	html, status, err := b.reply()
	return html, status, source, err
}

func (b *browserConverter) requests() []string {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func TestGatewayEscalatesProviderChallengeToBrowser(t *testing.T) {
	...
	if calls := browser.requests(); len(calls) != 1 {
		t.Fatalf("browser calls = %v, want exactly one", calls)
	}
}
...
// bytes, so doing it would burn a browser launch to arrive at a guaranteed
// failure.
func TestGatewayNeverSpendsBrowserRungOnBinaryDownload(t *testing.T) {
	...
	if calls := browser.requests(); len(calls) != 0 {
		t.Fatalf("browser calls = %v, want NONE for a binary download", calls)
	}
}
...
func TestProviderSearchFailsAtTheChallengeBody(t *testing.T) {
	...
	if calls := browser.requests(); len(calls) != 0 {
		t.Fatalf("browser calls = %v, want none for a discovery search", calls)
	}
}
