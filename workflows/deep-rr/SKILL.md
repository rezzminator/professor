...
**Preflight — the fetch MCP (professor's harvester family) must be live.** RR fetches only through `mcp__professor__harvester_read` (web pages in `urls`, papers and books in `publications`) (built-in WebFetch is hook-denied), so a missing or dead server makes every fetch error and yields a snippet-only run. Before launching, check for the `mcp__professor__harvester_*` tools (via ToolSearch):

- **Missing** — hold the launch and tell the user to enable pfm's harvester family (`pfm mcp harvester enable`, then `pfm install --yes`), then `/mcp` to connect.
- **Present** — smoke-test with one `mcp__professor__harvester_read` with a stable URL in `urls` (`https://example.com`); a clean read means go, an error means have the user reconnect (`/mcp`) or restart the server — hold until it passes.
