...
An unreadable fact renders as a failure to look, never as zero or empty: `tools ?`, `cache ?`, `role ?`. The cause goes to stderr, one line per row. This covers a payload with no session transcript, a missing file, a torn meta file and a meta file without `agentType`.
...
| Surface | File | Holds |
| --- | --- | --- |
| The row renderer | `pfm/internal/statusline/subagents.go`, `session_effort.go`, `model_segment.go`, `palette.go` | fields, order, effort, colours |
