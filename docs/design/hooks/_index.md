...
| callmeter | [callmeter.md](callmeter.md) | The async hooks (`PostToolUse`, `PostToolUseFailure`, `PostToolBatch`, `SubagentStart`, `SubagentStop`, `Stop`) that record every tool call, its bytes and its request's context size into SQLite; the command parsing; the reports on files, commands, context and repeated sequences; the transcript backfill |
