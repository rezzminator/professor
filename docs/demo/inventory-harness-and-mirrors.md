...
### B.3 `.claude/scripts/build-opencode.mjs` — `.claude/**` → `.opencode/**`

Header (:1-26): `.claude/agents/*.md → .opencode/agent/{name}.md`; `.claude/commands/**/*.md → .opencode/command/{flat-name}.md`; `.claude/skills/*/ → .opencode/skills/{name}` (symlink); `.mcp.json →` managed mcp object in `.opencode/opencode.jsonc`. Modes (:54-57) `generate`/`check`/`doctor`. Marker (:61) `Generated by .claude/scripts/build-opencode.mjs from {src}` — spot-checked at `.opencode/agent/dev.md:2`, `gitter.md:2`, `.opencode/command/git.md:2`, `dev.md:2`. Denies in `.opencode/opencode.jsonc:7-26`: edit deny `**/.claude/**`, `AGENTS.md`, `**/AGENTS.md`, `CLAUDE.md`, `**/CLAUDE.md`, `.opencode/**`; bash deny `git commit*`, `git push*`, `git tag*`, `gh release*`.

Trigger chain: `.claude/settings.json:57-73` Stop hook → `guard-stamp.sh stop` + `codex-sync.sh sync` (timeout 60). `codex-sync.sh`: `mark` (PostToolUse Edit|Write sets `tmp/professor_codex_dirty`), `sync` (Stop: `pfm codex build` + `check` AND `build-opencode.mjs generate` + `check`; failure warns at turn end, exit 1, names the failed stage; the flag stays set so the next turn retries). `.claude/scripts/dev.sh:293-297` runs `build-opencode.mjs check && doctor`, failing with "opencode mirror FAILED — run: node .claude/scripts/build-opencode.mjs generate".
...
- `.claude/{agents,commands,skills}` + `.mcp.json` → `build-opencode.mjs` → `.opencode/**` + `opencode.jsonc` mcp; `.mcp.json` → `codexgen/mcp.go` → `config.toml` mcp block.
- Both mirrors → `pfm codex check` / `build-opencode.mjs check|doctor` → terminal; on failure `codex-sync.sh sync` warns at Stop (exit 1, flag kept).
...
- `.professor/manifest.json` ("tracks 18 .opencode/** + build-opencode.mjs hashes") — not independently verified.

Excluded by scope: `releases/v0.60.0–v0.61.1.md`; `.worktrees/*/.claude/scripts/build-opencode.mjs` duplicates.
