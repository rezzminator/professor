...
| `pfm codex agents [--home PATH]` — no positional | JAIL | compiles `{home}/.professor/templates/global/agents/*.md`, never beside the source: the `.md` is linked into `{home}/.claude/agents` and the compiled TOML is written into each configured Codex home's `agents/` as a REGULAR FILE — Codex opens a role with `O_NOFOLLOW` and rejects a symlink as "agent type is currently not available", so a linked role is one that cannot spawn. Ownership is the generated marker on the file's first line: a pfm-written role is rewritten when it drifts, a pre-migration pfm symlink is replaced by the real file, and an unmarked file of the same name is a named conflict this never touches; a variant declared in `templates/global/agents/variants.json` (`{name: {from, <frontmatter key>: value}}`) is the source re-rendered with `name:` and the declared keys overridden, body byte-identical, into `paths.GeneratedClaudeAgentsDir`, then linked into every account and compiled to its own TOML like an original — a malformed declaration (unknown `from`, a key the source lacks, a name colliding with an original) aborts before any write; doctor counts a declared variant as owed and reports one row per Codex home, opening each role the way Codex does (no-follow, regular file) and comparing it to what the binary compiles — `installed` / `SYMLINK` / `MISMATCH` / `MISSING` / `UNREADABLE`; uninstall retires the marker-owned role files, the variant links and the retired generated directory; a link still pointing at the retired in-clone twin migrates on the next run; escaping mirrors build-codex.mjs:151-153 byte-for-byte (verified against the retired host `build-global-agents.py`, since deleted) | `internal/codexgen/globalagents_test.go`, `internal/codexgen/globalrole_test.go`, `internal/codexgen/globalvariants_test.go`, `cmd/pfm/codex_agents_command_test.go` |
...
| harness-prompt embed vs the clone (`pfm/harness-prompts/`) | JAIL | trees equal is `embed=ok`; any differing, clone-only or binary-only file is `embed=MISMATCH` naming each one and stating BOTH directions — a hash difference carries no chronology, so the row never prescribes a rebuild as though it knew which side moved — a warning and rc 1; a clone tree that cannot be read is `embed=CHECK FAILED`, a warning and rc 1 — never `ok`, never silence. The clone's own `*.go` sources and `.`/`_` names are not templates and are ignored | `internal/doctor/harness_prompt_embed_test.go` |
