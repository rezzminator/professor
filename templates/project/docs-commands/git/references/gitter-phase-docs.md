...
Invoked **after the main-loop session finishes merging permanent docs**, or after a flight lands. The caller passes `Archive:` — pipeline dirs to archive after committing, or `none`.
...
The `docs/dev/` pipeline dirs commit here too (git history is their permanent archive). Stage only doc paths:
...
Skip if `Archive: none`. `Archive:` entries may be dirs or single files. Per entry: `docs/dev/builds/*` → `tmp/dev/archive/builds/`. `tmp/` is gitignored — entries stay browseable while git history keeps the committed record; no archive remains under `docs/`.

```bash
mkdir -p tmp/dev/archive/builds
mv {entry} tmp/dev/archive/builds/
```
