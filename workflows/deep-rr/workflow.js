...
2. SIZE — call mcp__harvester__readPage (web urls) / mcp__harvester__readWork (works: DOI, id, findWorks handle) with size_only:true on EVERY candidate across all lanes in ONE parallel batch. With size_only it fetches + caches the full text and returns {size in tokens, path to the cache file, chars} and NO body. Drop any candidate that failed or came back walled/thin and pick another from the same lane.
